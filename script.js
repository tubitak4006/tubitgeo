...
(() => {
    ...
    function initForm() {
        ...
        // Şube auto-formatting (uppercase letter, slash insertion)
        const sube = form.querySelector('#sube');
        sube.addEventListener('input', (e) => {
            let v = e.target.value.toLocaleUpperCase('tr-TR').replace(/\s/g, '');
            // strip anything that's not digit, slash, or letter
            v = v.replace(/[^0-9/A-ZÇĞİÖŞÜ]/g, '');
            // auto-insert slash after 1-2 digits if a letter is typed next
            if (/^\d{1,2}[A-ZÇĞİÖŞÜ]/.test(v)) {
                v = v.replace(/^(\d{1,2})([A-ZÇĞİÖŞÜ])/, '$1/$2');
            }
            e.target.value = v.slice(0, 5);
        });

        // Field-line activation on input (keeps gradient visible while typing)
        ...
        function validate(formData) {
            ...
            const soyad = (formData.get('soyad') || '').trim();
            const subeVal = (formData.get('sube') || '').trim();

            if (ad.length < 2) errors.push({ field: 'ad', msg: 'Adınızı girin.' });
            if (soyad.length < 2) errors.push({ field: 'soyad', msg: 'Soyadınızı girin.' });
            if (!/^\d{1,2}\/[A-ZÇĞİÖŞÜ]$/.test(subeVal)) {
                errors.push({ field: 'sube', msg: 'Şubeyi 10/D biçiminde girin.' });
            }
            return { errors, ad, soyad, sube: subeVal };
        }
        ...
        form.addEventListener('submit', (e) => {
            ...
            if (result.errors.length) {
                ...
                const firstBad = form.querySelector('#' + result.errors[0].field);
                if (firstBad) firstBad.focus();
                return;
            }

            // Brief inline confirmation, then transition to workspace
            form.style.display = 'none';
            success.hidden = false;
            successName.textContent = result.ad;
            successSube.textContent = result.sube;

            // small pause so the success orb is visible, then transition
            setTimeout(() => {
    ...
    function transitionToWorkspace(user) {
        ...
        if (subeEl) subeEl.textContent = user.sube;
    ...
    function transitionToLanding() {
        ...
        // reset the registration form so a different user can sign in
        const form = document.getElementById('registrationForm');
        const success = document.getElementById('formSuccess');
        if (form) {
            form.reset();
    ...
    function initWorkspace() {
        ...
        // Seed with one demo function so the canvas isn't blank
        try {
            plotter.addFunction('x^2 - 4', nextColor());
            renderList();
        } catch {}
