...
(() => {
    ...
    /* ------------------------------------------------------
       3. Form handling — registration + login with account
          persistence (localStorage)
       ------------------------------------------------------ */

    /* ---- Account storage layer (localStorage) -------------- */
    const ACCOUNTS_KEY = 'gck:accounts';
    const CURRENT_USER_KEY = 'gck:currentUser';
    ...
    function saveAccounts(list) {
        ...
        catch (e) { console.warn('Hesap kaydedilemedi:', e); }
    }
    ...
    function setCurrentUser(acc) {
        try { localStorage.setItem(CURRENT_USER_KEY, JSON.stringify(acc)); }
        catch {}
    }
    ...
    function saveUserWork(acc, work) {
        try { localStorage.setItem(workKeyFor(acc), JSON.stringify(work)); }
        catch (e) { console.warn('Çalışma kaydedilemedi:', e); }
    }
    ...
    function initForm() {
        ...
        function flashError(input) {
            input.style.borderBottomColor = '#E11D48';
            input.addEventListener('input', () => { input.style.borderBottomColor = ''; }, { once: true });
            input.addEventListener('change', () => { input.style.borderBottomColor = ''; }, { once: true });
        }
        ...
        registerForm.addEventListener('submit', (e) => {
            ...
            // Prevent duplicate registration — direct user to login instead
            if (findAccount(candidate)) {
            ...
            // Brief success state, then transition
            registerForm.style.display = 'none';
            loginForm.style.display    = 'none';
            success.hidden = false;
            successName.textContent   = record.ad + ' ' + record.soyad;
            successSube.textContent   = record.sinifDerecesi + '/' + record.sube;
            successAction.textContent = 'kaydın başarıyla alındı.';

            setTimeout(() => { transitionToWorkspace(record); }, 800);
        });
        ...
        loginForm.addEventListener('submit', (e) => {
            ...
            loginForm.style.display    = 'none';
            success.hidden = false;
            successName.textContent   = existing.ad + ' ' + existing.soyad;
            successSube.textContent   = existing.sinifDerecesi + '/' + existing.sube;
            successAction.textContent = 'tekrar hoş geldin, çalışmaların yükleniyor.';

            setTimeout(() => { transitionToWorkspace(existing); }, 800);
        });

        /* --- Reset (start a new registration) --- */
        if (resetBtn) {
            resetBtn.addEventListener('click', () => {
                ...
                showMode('register');
                clearCurrentUser();
                registerForm.querySelector('#ad').focus();
            });
        }
    ...
    function transitionToWorkspace(user) {
        ...
        // Restore this user's saved functions (if any), else seed a demo
        const saved = loadUserWork(user);
        if (workspaceController && workspaceController.restoreFunctions) {
            workspaceController.restoreFunctions(saved);
        }
    ...
    function transitionToLanding() {
        // Save current user's work before leaving
        ...
        const loginForm    = document.getElementById('loginForm');
        const success      = document.getElementById('formSuccess');
        if (registerForm) { registerForm.reset(); registerForm.style.display = ''; }
        if (loginForm)    { loginForm.reset();    loginForm.style.display    = ''; }
    ...
    function initWorkspace() {
        ...
        // Persist current functions for the logged-in user
